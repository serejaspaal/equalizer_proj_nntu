...
module cmult_both_coupl_tb;
    ...
    initial begin
        ...
        x0 = -8; y0 = -8; x1 = -32; y1 = -32; @(posedge clk);

        if (out_re !== -7 || out_im !== 28) begin
            $error("Test1 FAIL: got (%0d,%0d), expected (-7,28)", out_re, out_im);
            errors++;
        ...
        @(posedge clk);
        if (out_re !== -39 || out_im !== 536) begin
            $error("Test4 FAIL: got (%0d,%0d), expected (-39,536)", out_re, out_im);
            errors++;
