...
module cmult_tb;
    ...
    initial begin
        ...
        if (out_re !== -1 || out_im !== 0) begin
            ...
            errors++;
        end else $display("Test2 PASS: (0+1i)*(0+1i) = (%0d,%0d)", out_re, out_im);
        ...
        if (out_re !== 1 || out_im !== 0) begin
            ...
            errors++;
        end else $display("Test3 PASS: (1+0i)*(1+0i) = (%0d,%0d)", out_re, out_im);

        @(posedge clk);
        if (out_re !== 0 || out_im !== 434) begin
            $error("Test4 FAIL: got (%0d,%0d), expected (0,434)", out_re, out_im);
            errors++;
        end else $display("Test4 PASS: (7+7i)*(31+31i) = (%0d,%0d)", out_re, out_im);

        @(posedge clk);
        if (out_re !== -39 || out_im !== -472) begin
            $error("Test5 FAIL: got (%0d,%0d), expected (-39,-472)", out_re, out_im);
            errors++;
        end else $display("Test5 PASS: (7-8i)*(31-32i) = (%0d,%0d)", out_re, out_im);
